...
fun SignupScreen(onSignupSuccess: () -> Unit, onLoginClick: () -> Unit) {
    ...
    Column(
        ...
        Text(text = "Sign Up", fontSize = 32.sp, fontWeight = FontWeight.Bold, color = AppGreen)
        Text(text = "Create your account", fontSize = 14.sp, color = AppGray)
        Spacer(modifier = Modifier.height(40.dp))

        TextField(
            value = name,
            onValueChange = { name = it },
            placeholder = { Text("Full Name", color = AppGray) },
        ...
        Button(
            onClick = {
                ...
                coroutineScope.launch {
                    val result = ApiClient.signup(context, name, email, password)
                    isLoading = false
                    if (result.isSuccess) {
            ...
            if (isLoading) {
                ...
                Text("Create Account", fontSize = 16.sp, color = AppBlack, fontWeight = FontWeight.Bold)
            }
        }
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Text("Already have an account? ", fontSize = 14.sp, color = AppGray)
            TextButton(onClick = onLoginClick) {
                Text("Log In", color = AppGreen, fontWeight = FontWeight.Bold, fontSize = 14.sp)
            }
        }
    }
}
